...
OWNER_ID = os.environ.get('OWNER_ID') or "0AYhsxb8/q9s9TMTGD0pSg=="
...
def incoming():
    def back_to_menu(db, quer, viber, delete_zakaz=True):
        ...
        viber.send_messages(viber_request.sender.id, [
            TextMessage(None, None,
                        'Здравствуйте, я помощник Юлии - вашего поставщика. Чем я могу вам помочь?'),
    ...
    if isinstance(viber_request, ViberMessageRequest):
        ...
        if quer.query_number == 'trac':
            ...
            sample_file['methodProperties']['Documents'][0]['DocumentNumber'] = viber_request.message.__getattribute__(
                'text')
        ...
        if quer.query_number == 'm5':
            ...
            if zkz.type in ['Кольцо', 'Набор', 'Браслет', 'Цепочка']:
                ...
                viber.send_messages(viber_request.sender.id, [
                    TextMessage(None, None,
                                'Напишите цвет изделия или отправьте фото. Если изделие без камней, просто поставьте прочерк')
                ])
                return Response(status=200)
        ...
        if quer.query_number == 'b1' and viber_request.message.__getattribute__('text') != np.city:
            ...
            i = 0
            city_string = viber_request.message.__getattribute__('text')
            send_city_message = 'false'
            # -*- coding: utf-8 -*-
            colors = ['#c1ef97', '#d7c3fe', '#feacae', '#f3e18f', '#aceaff', '#e7ba91', '#f2abf1']
            if 'Николаев' in city_string or 'Миколаїв' in city_string or 'Александровка' in city_string or 'Бар' in city_string:
                send_city_message = 'true'
                viber.send_messages(viber_request.sender.id, [TextMessage(None, None, 'Выберите порядковый номер города (укажите только цифру)')])


            city_list_buttons = {'Type': 'keyboard', 'Buttons': []}
            if send_city_message == 'true':
                for city in scr:
                    viber.send_messages(viber_request.sender.id, [
                        TextMessage(None, None, f'{i + 1}. {city.description}')
                    ])
                    i += 1
            else:
                for city in scr:
                    city_list_buttons['Buttons'].append({})
                    city_list_buttons['Buttons'][i]['Columns'] = 6
                    city_list_buttons['Buttons'][i]['Rows'] = 2
                    city_list_buttons['Buttons'][i]['Text'] = city.description
                    city_list_buttons['Buttons'][i]['TextSize'] = 'medium'
                    city_list_buttons['Buttons'][i]['TextHAlign'] = 'center'
                    city_list_buttons['Buttons'][i]['BgColor'] = random.choice(colors)
                    city_list_buttons['Buttons'][i]['ActionBody'] = i + 1
                    city_list_buttons['Buttons'][i]['ActionType'] = 'reply'
                    i += 1
            if send_city_message == 'false':
                if len(city_list_buttons['Buttons'][0]['Text']) > 0:
                    viber.send_messages(viber_request.sender.id, [
                        TextMessage(None, None, 'Выберите город'),
                        KeyboardMessage(keyboard=city_list_buttons),
                    ])
                else:
                    viber.send_messages(viber_request.sender.id, [
                        TextMessage(None, None, 'Неверно указан город. Попробуйте еще раз'),
                    ])
        ...
        if quer.query_number == 'm10':
            ...
            if np.adress:
                ...
                viber.send_messages(viber_request.sender.id, [
                    TextMessage(None, None, 'Неверно указан номер Новой Почты')
                ])
            return Response(status=200)
        ...
        if quer.query_number == 'm11':
            usr = User.query.filter_by(user_viber_id=viber_request.sender.id).first()
            np = NP.query.filter_by(user=usr).first()
            phone_string = viber_request.message.__getattribute__('text').replace(" ", "").replace("+", "")
            if len(phone_string) == 12 and phone_string[0] == '3' and phone_string[1] == '8' and phone_string[2] == '0':
                ...
                viber.send_messages(viber_request.sender.id, [
                    TextMessage(None, None, 'Неверно указан номер телефона')
                ])
            return Response(status=200)
        if quer.query_number == 'm12':
            ...
            try:
                ...
                with open('./bot/np_sample/create_person.json', 'w') as outfile:
                    json.dump(sample_file, outfile)
                ...
                viber.send_messages(viber_request.sender.id, [
                    TextMessage(None, None,
                                'Допущена ошибка в введении имени получателя. Чтобы начать работу с ботом напишите /reset')
                ])
            try:
        ...
        if quer.query_number == 'm16':
            ...
            try:
                ...
                viber.send_messages(viber_request.sender.id, [TextMessage(None, None,
                                                                          'Допущена ошибка в введении данных '
                                                                          'получателя. Напишите /reset и заполните '
                                                                          'все сначала')])
                viber.send_messages(viber_request.sender.id, [TextMessage(None, None, response.json())])
            db.session.commit()
            if np.type == 'Наложеный платеж':
                with open('./bot/np_sample/naloj-send.json') as file:
                    sample_file = json.load(file)
                sample_file['methodProperties']['BackwardDeliveryData'][0]['RedeliveryString'] = str(
                    np.oplata_nalojeniy)
            else:
            ...
            try:
                ...
                cost = response.json()["data"][0]['CostOnSite']
            except:
                viber.send_messages(viber_request.sender.id, [
                    TextMessage(None, None,
                                'Допущена ошибка в введении данных Новой Почты. Чтобы начать работу с ботом напишите /reset')
                ])
                viber.send_messages(viber_request.sender.id, [TextMessage(None, None, response.json())])
                return Response(status=200)
            viber.send_messages(viber_request.sender.id, [
                TextMessage(None, None, 'Спасибо за заказ! Номер ТТН: ' + str(
                    ttn) + '. Номер станет активным после отправки заказа. Отследить статус посылки вы можете '
                           'воспользовавшись кнопкой «Отследить ТТН»')
            ])
            ...
            message += f" - {ttn}"
            if np.oplata_dostavki == 'Sender':
                message += f"(+ доставка {cost} грн)"
            viber.send_messages(OWNER_ID, [
        ...
        if quer.query_number == 'm17':
            ...
            viber.send_messages(viber_request.sender.id, [
                TextMessage(None, None,
                            'Должна ли я вам вернуть некую сумму денег, после получения наложенного платежа?'),
                KeyboardMessage(keyboard=button),
            ])
            return Response(status=200)
        ...
        if quer.query_number == 'm18':
            if viber_request.message.__getattribute__('text') == 'Yes':
                ...
                viber.send_messages(viber_request.sender.id, [
                    TextMessage(None, None,
                                'Напишите сумму, котрую я должна вам вернуть после получения наложенного платежа')
                ])
                return Response(status=200)
            if viber_request.message.__getattribute__('text') == 'No':
        ...
        try:
            viber.send_messages(user_viber_id, [
                TextMessage(None, None,
                            'Напишите любой текст для начала. Что бы все сбросить и начать сначала напишите /reset')
            ])
        except:
            pass
        return Response(status=200)
